...
df = pd.read_csv('data/imdb_short.csv')  
...
for epoch in range(num_epochs):
    ...
    for idx, (name, module) in enumerate(model_prepared.named_modules()):
        if hasattr(module, 'weight') and module.weight is not None:
            ...
            weight_min = original_weight.min().item()
            weight_max = original_weight.max().item()

            clipping_min = weight_min
            clipping_max = weight_max + 1
